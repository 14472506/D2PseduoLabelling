...
from pseudo_labeling.solver.losses import affinity_loss, dice_loss

__all__ = [
    "CustomBaseMaskRCNNHead",
...
def mask_rcnn_loss(pred_mask_logits: torch.Tensor, instances: List[Instances], vis_period: int = 0):
    ...
    for instances_per_image in instances:
        ...
        if "gt_metric_score" in instances_per_image.get_fields().keys():
            gt_metric_scores_per_image = instances_per_image.gt_metric_score.to(dtype=torch.float64)
            gt_metric_scores.append(gt_metric_scores_per_image)
...
class CustomBaseMaskRCNNHead(nn.Module):
    ...
    def forward(self, x, instances: List[Instances]):
        ...
        x = self.layers(x)
        if self.training:
            mask_loss = mask_rcnn_loss(x, instances, self.vis_period)
